...
local function GetItems(filter)
    ...
    for _, obj in pairs(FindObjects(65536, "BlueprintGeneratedClass", "", 0, 0, false) or {}) do
        if obj and obj:IsValid() then
            ...
            if hasSubstring(path, {"/Buy", "/BP_Purchase", "/Purchase", "APMod"}) then
                local name = namefy(path)
                if name and (not filter or name:lower():find(filter:lower())) then
                    table.insert(out, path)
                end
...
local function processItemCommand(FullCommand, Parameters, Ar, callback)
    ...
    if not filter then
        ...
        print(consolefy2(items))
        print(string.format("Usage: %s <name, class name, or substring>", FullCommand))
        return true
    end
    ...
    if path then
        ...
        print(string.format("%s [%s] (%s)", err or "OK", tagify(path), namefy(path)))
    else
        print(consolefy2(items))
    end
...
local function spawn(path)
    local item = spawnObject(path)
end

for cmd,fn in pairs({add=AddItem, give=AddItem, drop=RemoveItem, remove=RemoveItem, chip=SpawnChip, egg=SpawnEgg, spawn=spawn}) do
...
RegisterConsoleCommandHandler("list", function(FullCommand, Parameters, Ar)
    ...
    for _, path in ipairs(items) do
        print(string.format("%s (%s)", tagify(path), namefy(path)))
    end
    return true
end)
