...
---@return AActor | nil
function GetObjectName(distance)
    ...
    local AddValue = GetKismetMathLibrary():Multiply_VectorInt(GetKismetMathLibrary():GetForwardVector(CameraManager:GetCameraRotation()), distance)
    ...
    if WasHit then
        return GetActorFromHitResult(HitResult)
    end
